  ...
  providers: [
    {
      ...
      useExisting: forwardRef(() => NgzSelectComponent),
      multi: true
    }
  ],
  selector: 'ngz-select',
  styleUrls: ['./ngz-select.styles.scss'],
  templateUrl: './ngz-select.template.html',
  host: {
...
export class NgzSelectComponent implements ControlValueAccessor, OnDestroy, AfterViewInit {
  ...
  constructor(public http: Http,
              private config: NgzSelectConfig,
              private el: ElementRef) {
  }

  inputKeyUpHandler(e: any) {
    if (e.keyCode === 9) {
      return;
    }

    this.onSearchModelChange.next(e);
  }
  ...
  writeValue(value: string | number, isSelect = false) {
    ...
    if (typeof value === 'string' || typeof value === 'number' && `${value}` !== '') {
      this.value = `${value}`;
      const array = this.isWithGroups ? this.flatMap(this._options, (item: any) => item.values) : this._options;
      const selectedOption: OptionModel = <OptionModel>array.find((option: OptionModel) => this.value === option.value);
      this.selectedItem = selectedOption;

      if (!selectedOption || !selectedOption.hasOwnProperty('label') || !selectedOption.hasOwnProperty('value')) {
        this.searchModel.setValue('');
      } else {
        const label = this.trimLabel(selectedOption.label);
  ...
  showResults(e: Event) {
    ...
    if (!this.isResultsShown) {
      if (!this.disableSearch) {
        this.el.nativeElement.querySelector('textarea').select();
      }
      this.filterOptions('');
  ...
  enterClickHandler(e: Event) {
    ...
    if (!this.isWithGroups && value) {
      const foundedItem: OptionModel = <OptionModel>this._options.find((el: OptionModel) => {
        return el.label.toLowerCase().trim().includes(value.toLowerCase().trim());
      });
      if (foundedItem) {
